...
namespace Engine.CommonImagery{
	...
	public class Polygon {
        // from Laurent Cozic : https://www.codeproject.com/Articles/15573/2D-Polygon-Collision-Detection

        private List<Vector> points = new List<Vector>();
		private List<Vector> edges = new List<Vector>();

        public Polygon(params Vector[] sommets) //TODO: vérif pour qu'il y ait au moins 3 sommets
        {
            foreach (Vector sommet in sommets)
            {
                points.Add(sommet);
            }
            BuildEdges(); //utile?
